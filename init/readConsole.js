...
let line = 1
const message = (question) => new Promise((resolve, reject) => {
...
const messageLoop = async (questionYN, question, defName = '', arr = []) => {
    ...
    if(want){
        let currentQuestion = `${question} (${defName+arr.length}) : `
        let value = await messageReplace(currentQuestion)
        if(!value) value = defName+arr.length
        arr.push(value)
        return await messageLoop(questionYN, question, defName, arr)
    }else return arr
}
