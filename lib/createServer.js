...
const createWithProtocol = (options, serverFunc) => {
...
class Server extends Routing {
  constructor(options) {
    super('/', null);
    this.server = createWithProtocol(options, (req, res) => {
  ...
  useWithBalancer(healthEndPoint = '/health/for/load/balancer/shvidko') { //in process
    this.get(healthEndPoint, (req, res) => {
      res.end(JSON.stringify({ isHealthy: true }));
    });
...
module.exports = { createServer, ifExistListen, createWithProtocol };
